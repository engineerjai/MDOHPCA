...
def CL_CD_CM_yaw(aerodynamic_outputs):
    """
       Calculates the stability derivivates that relate the lift, drag and 
       pitching moment coefficients to the yaw angle (Phi) of the wing; 
       .i.e. d(CL)/dPhi, d(CL)/dPhi and d(CM)/dPhi.
    ...
    wing_span = aerodynamic_outputs["span"]
    wing_sweep = aerodynamic_outputs["wing sweep"]
    CL_y0 = aerodynamic_outputs["CL"]
    CD_y0 = aerodynamic_outputs["CD"]
    CM_y0 = aerodynamic_outputs["CM"]
    wing_area = aerodynamic_outputs["S_ref"]
    naca_series = aerodynamic_outputs["NACA"]
    aver_chord = (root_chord+tip_chord)/2
    ...
    "twist_cp":np.array([tip_twist,
                          root_twist]),      # Initializes the twist angles of the wing
    "mesh":mesh,
    "CL0":CL_y0,                             # CL of the surface at yaw angle = 0
    "CD0":CD_y0,                             # CD of the surface at yaw angle = 0  
    "k_lam":0.05,                            # Percentage of chord with laminar flow used for viscous analysis
    "t_over_c_cp":np.array([t_over_c_max,
                    t_over_c_max]),          # Thickness-to-chord ratio at control points (i.e. root and tip)
    "c_max_t":c_max_t,                       # Chordwise location of maximum thickness
    ...
    prob_der = om.Problem()
    indep_var_comp = om.IndepVarComp()
    indep_var_comp.add_output("yaw", val=10, units="deg")                       # This is the aim of the analysis: defines a yaw angle   
    indep_var_comp.add_output("v", val=-cruise_speed, units="m/s")              # Cruise speed relative to outside air
    indep_var_comp.add_output ("alpha", val=0, units="deg")                     # Angle of attack is zero
    indep_var_comp.add_output("Mach_number", val=cruise_speed/a_cruise)         # 'True' Mach number at cruise conditions
    indep_var_comp.add_output("re", val=Re_cruise, units="1/m")                 # Reynolds number at cruise conditions
    indep_var_comp.add_output("rho", val=cruise_air_density, units="kg/m**3")   # Air density at cruise altitude
    indep_var_comp.add_output("cg", val=np.zeros(3), units="m")                 # Initialization of the centre of gravity vector
    ...
    prob_der.model.add_design_var(point_name + ".wing_perf.t_over_c", lower=0, upper=2) # although not exactly a design variable, oas doesn't work without this
    prob_der.model.add_constraint(point_name + ".wing.S_ref", lower=0, upper=wing_area, units="m**2")
    prob_der.model.add_design_var("alpha", lower=0, upper=10, units="deg")
    prob_der.model.add_objective(point_name + ".wing_perf.CD", scaler=1e4)
    ...
    # print("CM = " + str(prob_der[point_name + ".total_perf.CM"][2]))
    # print("AoA = " + str(prob_der["alpha"][0]))
    # print("Yaw = " + str(prob_der["yaw"][0]))
    
    CL_y10 = prob_der[point_name + ".wing_perf.CL"][0]
    CD_y10 = prob_der[point_name + ".wing_perf.CD"][0]
    CM_y10 = prob_der[point_name + ".total_perf.CM"][1]
    
    
    CL_psi = (CL_y10 - CL_y0)/(10/180*np.pi)           # [per radian]
    CD_psi = (CD_y10 - CD_y0)/(10/180*np.pi)           # [per radian]
    CM_psi = (CM_y10 - CM_y0)/(10/180*np.pi)           # [per radian]

    
    return CL_psi, CD_psi, CM_psi
    
    

def CL_CD_CM_roll(aerodynamic_outputs):
    """
       Calculates the stability derivivates that relate the lift, drag and 
       pitching moment coefficients to the roll angle (Psi) of the wing; 
       .i.e. d(CL)/dPsi, d(CL)/dPsi and d(CM)/dPsi.
    ...
    wing_span = aerodynamic_outputs["span"]
    wing_sweep = aerodynamic_outputs["wing sweep"]
    CL_r0 = aerodynamic_outputs["CL"]
    CD_r0 = aerodynamic_outputs["CD"]
    CM_r0 = aerodynamic_outputs["CM"]
    wing_area = aerodynamic_outputs["S_ref"]
    naca_series = aerodynamic_outputs["NACA"]
    aver_chord = (root_chord+tip_chord)/2
    ...
    "twist_cp":np.array([tip_twist,
                          root_twist]),      # Initializes the twist angles of the wing
    "mesh":mesh,
    "CL0":CL_r0,                             # CL of the surface at roll angle = 0
    "CD0":CD_r0,                             # CD of the surface at roll angle = 0  
    "k_lam":0.05,                            # Percentage of chord with laminar flow used for viscous analysis
    "t_over_c_cp":np.array([t_over_c_max,
                    t_over_c_max]),          # Thickness-to-chord ratio at control points (i.e. root and tip)
    "c_max_t":c_max_t,                       # Chordwise location of maximum thickness
    ...
    prob_der = om.Problem()
    indep_var_comp = om.IndepVarComp()
    indep_var_comp.add_output("roll", val=10, units="deg")                      # This is the aim of the analysis: defines a roll angle   
    indep_var_comp.add_output("v", val=-cruise_speed, units="m/s")              # Cruise speed relative to outside air
    indep_var_comp.add_output ("alpha", val=0, units="deg")                     # Angle of attack is zero
    indep_var_comp.add_output("Mach_number", val=cruise_speed/a_cruise)         # 'True' Mach number at cruise conditions
    indep_var_comp.add_output("re", val=Re_cruise, units="1/m")                 # Reynolds number at cruise conditions
    ...
    # print("CM = " + str(prob_der[point_name + ".total_perf.CM"][2]))
    # print("AoA = " + str(prob_der["alpha"][0]))
    # print("Roll = " + str(prob_der["roll"][0]))
    
    CL_r10 = prob_der[point_name + ".wing_perf.CL"][0]
    CD_r10 = prob_der[point_name + ".wing_perf.CD"][0]
    CM_r10 = prob_der[point_name + ".total_perf.CM"][1]
    
    
    CL_phi = (CL_r10 - CL_r0)/(10/180*np.pi)           # [per radian]
    CD_phi = (CD_r10 - CD_r0)/(10/180*np.pi)           # [per radian]
    CM_phi = (CM_r10 - CM_r0)/(10/180*np.pi)           # [per radian]

    
    return CL_phi, CD_phi, CM_phi

       

class coefficients:
    def __init__(self, aerodynamic_outputs):
        self.CL_alpha, self.CD_alpha, self.CM_alpha = CL_CD_CM_alpha(aerodynamic_outputs)
        self.CL_phi, self.CD_phi, self.CM_phi = CL_CD_CM_roll(aerodynamic_outputs)
        self.CL_psi, self.CD_psi, self.CM_psi = CL_CD_CM_yaw(aerodynamic_outputs)
